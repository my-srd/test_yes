...
public function widget( $args, $instance ) {
		...
		echo '<div class="item '.esc_attr($active).' ">';
		   echo '<a href="'.esc_url($slides['link']).'"><img class="img-responsive" src="'.esc_url($slides['image_url']).'" alt="Image" style="width:100%;height:'.absint($max_height).'px"></a>';
	
		$link = $btn_url;
		if ($btn_url=='') $link = $slides['link'];
		
		if( $product_meta ||  $title_text) {   
			echo '<div class="carousel-caption">
				  <h1>'.esc_html($slides['title']).'</h1>
				  <p>'.wp_kses_post($slides['content']).'</p>
				  <a href='.esc_url($link).' class="call-to-action btn">'.esc_html($btn_label).'</a><p></p>
				</div>';
		}				   
		 $active = '';
		 echo '</div>';
...
public function form( $instance ) {
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'title_text' )); ?>"><?php esc_html_e( 'Title & Description', 'ecommerce-plus' ); ?></label> 
		</p>
